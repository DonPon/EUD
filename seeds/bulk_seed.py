...
def bulk_seed(num_clients=20, num_le_clients=10):
    ...
    for i in range(num_clients):
        ...
        PersonalInformation.objects.create(
            ...
            type_of_relationship=random.choice(['Owner', 'Co-owner', 'POA (general)', 'Beneficial Owner']),
            first_and_last_name=fake.name(),
            first_name=fake.first_name(),
            last_name=fake.last_name(),
            name_at_birth=fake.last_name(),
            type_id_NCI=random.choice(['Passport', 'National ID']),
            fiscal_it_number=fake.bothify(text='IT-########'),
            type_id_document=random.choice(['Passport', 'ID Card']),
            release_authority=fake.company(),
            release_place=fake.city(),
            release_date=fake.past_date(start_date='-5y'),
            expiry_date=fake.future_date(end_date="+5y"),
            copy_id_provided=random.choice(['Yes', 'No']),
            ...
            has_ebanking=random.choice(['Yes', 'No']),
