...
class BulkDeleteLEView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        if client_ids:
            ...
            # Otherwise, deleting from LE_BankingRelationship first would make the QuerySet empty
            # for subsequent models in the loop.
            client_uuids = list(LE_BankingRelationship.objects.filter(id__in=client_ids).values_list('client_uuid', flat=True))
            
            if client_uuids:
                from django.apps import apps
                le_app = apps.get_app_config('clients_le')
                for model in le_app.get_models():
...
class LE_ClientDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for name, details in registry.items():
            ...
            if name not in ['le_bankingrelationship', 'le_company']:
                model = details['model']
